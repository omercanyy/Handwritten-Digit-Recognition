...
fontSize = 16;
set(0, 'DefaultFigureWindowStyle','docked'); % For the images to be docked already.
...
hGreenCurve = plot(finalX, finalY, '-y');
    hBW = imfilter(grayImage, ones(2,2));
    minVal = min(hBW(:));
    maxVal = max(hBW(:));
    diff = maxVal - minVal;
...
%load('Trained_NN.mat');
%D = imfilter(grayImage, ones(2,2));
%fprintf('The digit you draw is %d\n.', mod(predict(Theta1, Theta2, D(:)'), 10));